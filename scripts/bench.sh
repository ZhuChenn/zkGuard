...
DEFAULT_LOCAL_WEIGHT_TEMPLATE="${PROJECT_ROOT}/program/benchmarks/zkguard-pallets-weight-template.hbs"
...
function usage {
    ...
    ZKV_NODE_EXE    : the path to the zkguard executable [target/production/zkguard in project root]
    ZKV_RUNTIME     : the path to the zkguard-runtime wasm [target/production/wbuild/zkguard-runtime/zkguard_runtime.compact.compressed.wasm in project root]
    "
    if [ -n "${message}" ]; 
    then
        echo "ERROR: $message"
